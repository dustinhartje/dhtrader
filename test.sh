...
for a in "$@"; do
    if [ "$a" = "-h" ] || [ "$a" = "--help" ] || [ "$a" = "-?" ]; then
        echo "To show full error output: test.sh -v"
        ...
        echo ""
        echo "Marks are listed in setup.cfg/pytest.ini or grep for '@pytest.mark'"
        exit 0
    fi
done
...
pytest -sv -rA --tb=short --durations=0 --show-capture=stdout "${EXTRA_MARKS[@]}" "${ARGS[@]}"
